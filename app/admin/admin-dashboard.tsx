...
export function AdminDashboard() {
  ...
  async function toggleEditorial(field: "irisSnapVerified") {
    ...
    setStandardMessage("");
    try {
      await updateEditorialSelection(selected.id, field, value);
      setSelected((current) => current?.id === selected.id ? { ...current, [field]: value } : current);
      setItems((current) => current.map((item) => item.id === selected.id ? { ...item, [field]: value } : item));
      setStandardMessage("Editorial selection saved. Public pages refresh within five minutes.");
    } catch (error) {
